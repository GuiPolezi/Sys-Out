...
export default function Dashboard() {
  ...
  const checkUserAndFetchSites = async () => {
    ...
    if (data) {
      setWebsites(data)
      await checkAllInBatches(data)
    }
  }

  // Verifica todos os sites em lotes paralelos
  const checkAllInBatches = async (sites: Website[]) => {
    for (let i = 0; i < sites.length; i += BATCH_SIZE) {
      const batch = sites.slice(i, i + BATCH_SIZE)
      await Promise.allSettled(
        batch.map((site) => checkStatus(site.id, site.url))
      )
    }
  }
  ...
  const addWebsite = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from('websites')
      .insert([{ url: newUrl, user_id: session.user.id }])
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={addWebsite} className="flex gap-4 mb-8">
          ...
          <button
            type="submit"
            className="bg-blue-600 text-white px-6 py-3 rounded shadow-sm hover:bg-blue-700 transition font-medium"
          >
            Adicionar Site
          </button>
        </form>
          ...
          {websites.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-200">
              {websites.map((site) => (
                <li
                  key={site.id}
                  className="p-6 flex flex-col gap-4 sm:flex-row justify-between items-center hover:bg-gray-50 transition-colors"
                >
                    ...
                    {results[site.id] && (
                      <span
                        className={`text-sm mt-1 font-medium ${
                          results[site.id].isOnline ? 'text-green-600' : 'text-red-600'
                        }`}
                      >
                        {results[site.id].isOnline ? '🟢 Online' : '🔴 Fora do ar'} —{' '}
